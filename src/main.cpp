...
using std::shared_ptr, std::make_shared, std::vector, std::string, std::cout,
    std::endl;
using namespace rev;
...
void field_centric() {
  ...
  SlipstreamPower power{.front_left_forward = robotFwd + turn + robotStrafe,
...
// eject the wrong colored object
...
void color_task() {
  ...
  while (true) {
    ...
    // pros::lcd::print(6, detected_color == Color::RED ? "RED" : detected_color
    // == Color::BLUE ? "BLUE" : "UNKNOWN");
    pros::delay(10);
  }
}
...
void anti_jam() {
  ...
  while (true) {
    ...
    if (!is_front_stalling &&
        intake.get_torque() > front_intake_torque_threshold) {
    ...
    if (is_front_stalling &&
        intake.get_torque() < front_intake_torque_threshold * 0.9) {
...
void intake_code() {
  ...
  if (!is_sorting) {
    ...
    // reverse, otherwise run the intakes based on the intake state, if the
    // intake is stalled, dont run it to prevent further stalling
...
void lever_code() {
  ...
  if (controller.get_digital(pros::E_CONTROLLER_DIGITAL_R1) ||
      lever_actuating || lever_retracting) {
    ...
    // intake state to in,
    // moving the lever to the retract position, and setting the hood to the
    // scoring position
    ...
    if (lever_actuating &&
        (lever.get_position() >= lever_end_position ||
         pros::millis() - score_press_time >= lever_timeout)) {
    ...
    if (lever_retracting &&
        pros::millis() - lever_up_time >= lever_retract_pause) {
...
void opcontrol() {
  ...
  while (true) {
    ...
    double left_x =
        controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_X) / 127.0;
    double right_x =
    ...
    // If the down button is pressed, set the scraper state to true, otherwise
    // set it to false
    ...
    // If the Y button is pressed, set the descore piston state to true,
