...
void MecanumToDistance::init(OdometryState initial_state) {
  ...
  headingPID = PID(p.heading_k.p, p.heading_k.i, p.heading_k.d,
                   p.heading_k.starti);
}

SlipstreamSegmentStatus MecanumToDistance::step(OdometryState current_state) {
  // odom.pos.y = field x (left/right), odom.pos.x = field y (forward)
  QLength dx = current_state.pos.y - start_pos.y;  // field x displacement
  QLength dy = current_state.pos.x - start_pos.x;  // field y displacement

  // Project displacement onto the target heading direction
  // heading=0 is forward (y+), heading=90 is right (x+)
  double heading_rad = heading.convert(radian);
  QLength traveled =
      dx * std::sin(heading_rad) + dy * std::cos(heading_rad);
  ...
  heading_output = std::clamp(heading_output, -p.turn_max_speed, p.turn_max_speed);
