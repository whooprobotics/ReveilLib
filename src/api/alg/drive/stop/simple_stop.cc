...
stop_state SimpleStop::get_stop_state(OdometryState current_state,
  ...
  QSpeed longitudinal_speed = sqrt(current_state.vel.xv * current_state.vel.xv +
                                   current_state.vel.yv * current_state.vel.yv);
  ...
  // Because of the previous step, this should result in the starting
  // longitudinal_distance being always (-d, 0) Successive errors should also be
  // of the form (x, y), where x < 0 and y is small
