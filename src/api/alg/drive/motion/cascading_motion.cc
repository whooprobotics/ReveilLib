...
rev::CascadingMotion::CascadingMotion(double ipower,
                                      ...
                                      QSpeed imax_v,
                                      double ik_v)
    : power(fabs(ipower)), k_p(fabs(ik_p)), k_b(fabs(ik_b)), max_v(abs(imax_v)), k_v(fabs(ik_v)) {}
...
std::tuple<double, double> rev::CascadingMotion::gen_powers(
  ...
  QSpeed v_target = max_v * (1 - exp(-k_v * err_y.convert(inch)));
  ...
  QSpeed v_target = max_v * (1 - exp(
    -k_v * abs(err_y).convert(inch)
  )) * sgn(err_y.get_value());
