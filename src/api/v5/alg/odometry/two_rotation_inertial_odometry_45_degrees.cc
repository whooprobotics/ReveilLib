...
OdometryState TwoRotationInertialOdometry45Degrees::get_state() {
  current_position_mutex.take(TIMEOUT_MAX);
  OdometryState ret = current_position;
  current_position_mutex.give();
  return ret;
}
void TwoRotationInertialOdometry45Degrees::set_position(Position pos) {
  current_position_mutex.take(TIMEOUT_MAX);
...
void TwoRotationInertialOdometry45Degrees::reset_position() {
...
void TwoRotationInertialOdometry45Degrees::step() {
  ...
  if (d_heading_ticks != 0) {
    ...
    local_off_lat =
        2 * sindt2 * (raw_side_translation / dht_radian + horizontal_distance_from_cog);
    local_off_long =
        2 * sindt2 *
        (raw_fwd_translation / dht_radian + vertical_distance_from_cog);
  } else {
