...
Motor::Motor(const std::int8_t port, const motor_gearset_e_t gearset)
    : port(abs(port)) {
  set_gearing(gearset);
  if (port < 0)
    set_reversed(true);
...
Motor::Motor(const std::int8_t port) : port(abs(port)) {
  if (port < 0)
    set_reversed(true);
}
...
std::int32_t Motor::get_raw_position(
...
std::uint8_t Motor::check_port(void) const {
