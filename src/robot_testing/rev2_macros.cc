...
void driveTo(double left_volts, double right_volts, rev::Drive params) {
  rev::SlipstreamPower power = {.front_left_forward = left_volts / 12,
                                .front_right_forward = right_volts / 12,
                                .rear_left_forward = left_volts / 12,
                                .rear_right_forward = right_volts / 12,

                                .front_left_steer = left_volts / 12,
                                .front_right_steer = right_volts / 12};

  PID timer(0, 0, 0, 0, 0, 0_ms, 0, 0_ms, 0, params.timeout);
  while (1) {
    timer.compute(0, 29_ms);
    if (timer.is_settled()) {
      break;
    }
    std::cout << timer.time_spent_running.convert(millisecond) << " ms" << std::endl;
    chassis->drive_holonomic(power);
    pros::delay(20);
  }
  chassis->drive_holonomic(0, 0, 0);
}
...
void drive() {
  double throttle = rev::deadband(
      controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_Y) / 127.0, 0.05);
  double strafe = rev::deadband(
      controller.get_analog(pros::E_CONTROLLER_ANALOG_LEFT_X) / 127.0, 0.05);
  double turn = rev::deadband(
      controller.get_analog(pros::E_CONTROLLER_ANALOG_RIGHT_X) / 127.0, 0.05);
...
bool eject_state = false;  // highest precednece, if this is true, the robot
                           // will reverse both intakes to clear all blocks out
bool outtake_state = false;  // if this is true, the robot will reverse the back
                             // intake and briefly reverse the front intake to
                             // score in low goal without jamming
bool intake_in_state =
    false;  // if this is true, the intakes will run forward to take in blocks,
            // if false, the intakes will stop,
...
void set_descore(bool state) {
  descore_piston.set_value(
      !state);  // set true input to down position but want false to be the
                // default so invert the input4
}
...
// Anti-jam system, if the torque on either intake exceeds the threshold for a
// certain amount of time, it cuts power to the intakes until the torque drops
// back down, indicating the jam has been cleared
void anti_jam() {
  ...
  while (true) {
    ...
    if (!is_back_stalling &&
        back_intake.get_torque() > back_intake_torque_threshold) {
    ...
    if (is_front_stalling &&
        front_intake.get_torque() < front_intake_torque_threshold * 0.9) {
      is_front_stalling = false;
    }
    if (is_back_stalling &&
        back_intake.get_torque() < back_intake_torque_threshold * 0.9) {
    ...
    // If either intake is stalled, set the is_stalling flag to true, otherwise
    // set it to false
...
void reset_jam() {  // Resets the anti-jam system, setting all the stalled
                    // variables to false
...
// Sorts the color, if the detected color is not the same as the team color and
// is not NONE, it runs the intake in reverse for a short amount of time to
...
static Color TEAM =
    Color::RED;  // default team color is red, can be set w this function
...
void color_sort(Color color, Color team_color) {
  ...
  if (color != team_color && color != Color::NONE) {
    if (!is_sorting)
      sort_total_start_time = pros::millis();
    is_sorting = true;
    back_intake.move_voltage(-12000);
    front_intake.move_voltage(6000);
    sort_start_time = pros::millis();
  }
  if (is_sorting && pros::millis() - sort_start_time > sort_timeout &&
      pros::millis() - sort_total_start_time > sort_total_timeout) {
    is_sorting = false;
  }
}
...
void intake_control() {
  // Intake control, if the intake is not currently sorting, allow the driver to
  // control the intake, otherwise ignore driver input to prevent interference
  // with the sorting process
  static int outtake_reverse_timer = 0;  // ms
  static bool outtaking = false;
  if (toggle_intake) {
    // Toggle the intake state, if the intake is currently stalled, reset the
    // anti-jam system instead of toggling the intake
    if (!is_stalled) {
  ...
  if (!is_sorting) {
    // If the left trigger is held, run both intakes in reverse to eject
    // objects, if the right trigger is held, run just the back intake in
    // reverse, but run the front intake in reverse for a short amount of time
    // to help eject the blocks otherwise run the intakes based on the intake
    // state, if the intake is stalled, dont run it to prevent further stalling
    if (eject_state) {
      ...
      front_intake.move_voltage(
          12000 * !front_intake_stalled);  // turn on if not stalled
      back_intake.move_voltage(12000 *
                               !back_intake_stalled);  // turn on if not stalled
    } else {
      back_intake.move(0);
      front_intake.move(0);
    }
...
void reset_lever() {
  ...
  while (true) {
    if (pros::millis() - lever_reset_start_time > lever_reset_timeout)
      break;
    if (lever.get_torque() > 0.2)
      break;
...
void lever_control() {
  // lever piston goes up immediately, lever follows 200ms later; lever piston
  // goes down 500ms after release
  ...
  // if score_shallow is true, we want to keep it true until the lever is done
  // moving to prevent it from changing mid-score, also if the score button is
  // released but the lever is still moving, we want to keep the score_shallow
  // variable true until the lever is done moving to prevent it from changing
  // mid-score
  score_shallow &= (score || lever_actuating || lever_retracting ||
                    lever_paused || lever_setting_up);

  // If the score button is pressed, start the scoring process, which involves
  // moving the lever and actuating the piston in a certain sequence with
  // specific timing to achieve the desired scoring motion, also set the hood to
  // the scoring position
  if (score || lever_actuating || lever_retracting || lever_paused ||
      lever_setting_up) {
    // If the lever is not currently moving, start the scoring process by
    // setting the score press time, resetting the anti-jam system, setting the
    // intake state to in, moving the lever to the retract position, and setting
    // the hood to the scoring position
    ...
    // After the retract pause time has gone by, move the lever back to the rest
    // position and lower the piston
    ...
    if (lever_retracting &&
        (lever.get_position() <= lever_rest_position + 10 ||
         pros::millis() - lever_retract_time >=
             lever_retract_timeout)) {  // if the lever is close enough to the
                                        // rest position, we can assume it has
                                        // finished retracting and reset the
                                        // retracting state just in case the
                                        // timing was off
      lever_retracting = false;
      score = false;  // reset score to prevent re-entering the scoring process
                      // without a new button press
  ...
  } else {  // If the R1 button is not pressed, reset the lever and piston to
            // the default positions
    if (!lever_score_fail && !lever_retract_fail)
      lever.move_absolute(lever_rest_position, lever_retract_velocity);
    // lever.move_voltage(0);
    lever_piston.set_value(0);
    recover_lever();
  ...
  if (!lever_actuating && !lever_retracting && !lever_paused &&
      !lever_setting_up &&
      pros::millis() - lever_retract_time >= hood_timeout) {
    set_hood(false);
  }
}
