...
const std::tuple<double, double, double> pid_constants = {bez_kp, bez_ki,
                                                          bez_kd};
const QLength wheelbase = 13_in;

std::vector<PointVector> robor_points{};
void update_robor_points(std::shared_ptr<rev::Reckless> reckless,
                         std::shared_ptr<rev::DriftlessSim> sim) {
...
TEST(Autons, sonic1) {
  ...
  reckless->go(RecklessPath().with_segment(PilonsSegment(
      std::make_shared<CascadingMotion>(1, kP, kB, 60_in / second, 0.07),
      std::make_shared<PilonsCorrection>(2, 0.5_in),
      std::make_shared<SimpleStop>(0_s, 0.2_s, 0.4), {0.25_ft, 5_ft, 0_deg},
      0_in)));
  ...
  reckless->go(
      RecklessPath()
          .with_segment(
              PilonsSegment(std::make_shared<CascadingMotion>(
                                1, kP, kB, 60_in / second, 0.07),
                            std::make_shared<PilonsCorrection>(2, 0.5_in),
                            std::make_shared<SimpleStop>(0_s, 0.2_s, 0.4),
          ...
          .with_segment(
              PilonsSegment(std::make_shared<CascadingMotion>(
                            ...
                            {1_ft, 4.5_ft, 0_deg}, 0_in))
          .with_segment(PilonsSegment(
              std::make_shared<CascadingMotion>(1, kP, kB, 60_in / second,
                                                0.07),
              std::make_shared<PilonsCorrection>(2, 0.5_in),
              std::make_shared<SimpleStop>(0_s, 0.2_s, 0.4),
