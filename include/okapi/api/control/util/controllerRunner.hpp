...
class ControllerRunner {
 public:
  ...
  virtual Output runUntilSettled(const Input itarget,
    ...
    LOG_INFO(
        "ControllerRunner: runUntilSettled(AsyncController): Done waiting to "
        "settle");
  ...
  virtual Output runUntilSettled(
    ...
    LOG_INFO(
        "ControllerRunner: runUntilSettled(IterativeController): Set target "
        "to " +
        std::to_string(itarget));
    icontroller.setTarget(itarget);
    ...
    LOG_INFO(
        "ControllerRunner: runUntilSettled(IterativeController): Done waiting "
        "to settle");
    return icontroller.getError();
  }
  ...
  virtual Output runUntilAtTarget(const Input itarget,
    ...
    LOG_INFO(
        "ControllerRunner: runUntilAtTarget(AsyncController): Done waiting to "
        "settle");
    return icontroller.getError();
  }
  ...
  virtual Output runUntilAtTarget(
      ...
      ControllerOutput<Output>& ioutput) {
    LOG_INFO(
        "ControllerRunner: runUntilAtTarget(IterativeController): Set target "
        "to " +
        std::to_string(itarget));
    icontroller.setTarget(itarget);
    ...
    LOG_INFO(
        "ControllerRunner: runUntilAtTarget(IterativeController): Done waiting "
        "to settle");
    return icontroller.getError();
  }
