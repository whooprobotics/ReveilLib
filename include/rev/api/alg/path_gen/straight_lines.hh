...
class StraightSegments : public PurePursuitSegment {
 public:
   ...
   * will take the shape of a tractrix curve as it is "dragged along" the path
   * at the look ahead distance. The smaller the look ahead distance, the closer
   * to being a straight line. Additionally, between each point, the robot will
   * curve towards the next line. Smaller look ahead distances will result in a
   * sharper curve (will start curving at the look ahead distance)
   *
   ...
   * @param resolution The resolution of the straight segment, if left at 0
   * becomes length of path points times 5. It is important not to set the
   * resolution too high as it can become too computationally intensive
   * @param look_ahead_distance The look ahead distance for the pure pursuit
   * algorithm (do not set below 3 inches)
   */
