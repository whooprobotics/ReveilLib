...
namespace rev {
/**
 * @brief Implementation of a motor under the AnyMotor interface
 * 
 */
class Motor : public AnyMotor {
 public:
  ...
  /**
   * @brief Stops the motor using the currently configured brake mode.
   *
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * @returns The motor's actual velocity in RPM or PROS_ERR_F if the operation
   * failed, setting errno.
   */
   ...
   * ENODEV - The port cannot be configured as a motor
   *
   * @note Although this function forwards data from the motor, the motor
   * presently does not provide any value. This function returns PROS_ERR with
   * errno set to ENOSYS.
   *
   * @returns 1 if the motor is not moving, 0 if the motor is moving, or PROS_ERR
   ...
   *
   * @returns The motor's port number.
   */
  ...
  /**
   * @brief Validates that the motor's port can be properly configured.
   * 
   * @returns 0 if the port is properly configured, otherwise returns the port number of the motor's port
   */
