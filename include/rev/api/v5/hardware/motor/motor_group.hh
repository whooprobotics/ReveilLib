...
namespace rev {
/**
 * @brief Implementation of the MotorGroup under the rev::AnyMotor interface
 */
class MotorGroup : public AnyMotor {
 public:
  ...
  /****************************************************************************/
  /**
   * @brief Sets the voltage for the MotorGroup from -127 to 127.
   *
  ...
  /**
   * @brief Sets the target absolute position for the MotorGroup to move to.
   *
   ...
   *
   * @note This function simply sets the target for the MotorGroup, it does not
  ...
  /**
   * @brief Sets the relative target position for the MotorGroup to move to.
   *
  ...
  /**
   * @brief Sets the velocity for the MotorGroup.
   *
  ...
  /**
   * @brief Sets the output voltage for the MotorGroup from -12000 to 12000 in
   * millivolts.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * @brief Stops the MotorGroup using the currently configured brake mode.
   *
  ...
  /**
   * @brief Changes the output velocity for a profiled movement (motor_move_absolute()
  ...
  /**
   * @brief Gets the actual velocity of the MotorGroup.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
   *
   * @returns The MotorGroup's actual velocity in RPM or PROS_ERR_F if the
   * operation failed, setting errno.
   */
  double get_actual_velocity(void) const override;

  /**
   * @brief Gets the direction of movement for the MotorGroup.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
  ...
  /**
   * @brief Checks if the MotorGroup is stopped.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
   *
   * @note Although this function forwards data from the MotorGroup, the
   * MotorGroup presently does not provide any value. This function returns
   * PROS_ERR with errno set to ENOSYS.
   *
   * @returns 1 if the MotorGroup is not moving, 0 if the MotorGroup is moving,
   * or PROS_ERR if the operation failed, setting errno
   */
  std::int32_t is_stopped(void) const override;

  /**
   * @brief Gets the raw encoder count of the MotorGroup at a given timestamp.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
  ...
  /**
   * @brief Gets the absolute position of the MotorGroup in its encoder units.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
   *
   * @returns The MotorGroup's absolute position in its encoder units or
  ...
  /**
   * @brief Sets the position for the MotorGroup in its encoder units.
   *
  ...
  /**
   * @brief Sets the "absolute" zero position of the MotorGroup to its current
   * position.
   *
   * This function uses the following values of errno when an error state is
   * reached:
  ...
  /**
   * @brief Sets one of motor_brake_mode_e_t to the MotorGroup.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
  ...
  /**
   * @brief Sets one of motor_encoder_units_e_t for the MotorGroup encoder.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
  ...
  /**
   * @brief Sets one of motor_gearset_e_t for the MotorGroup.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
  ...
  /**
   * @brief Sets the reverse flag for the MotorGroup.
   *
  ...
  /**
   * @brief Gets the brake mode that was set for the MotorGroup.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
   *
   * @returns One of motor_brake_mode_e_t, according to what was set for the
  ...
  /**
   * @brief Gets the encoder units that were set for the MotorGroup.
   *
   * This function uses the following values of errno when an error state is
   * reached:
   * ENODEV - The port cannot be configured as a MotorGroup
   *
   * @returns One of motor_encoder_units_e_t according to what is set for the
  ...
  /**
   * @brief Validates that all ports provided can be properly configured.
   * 
   * @returns 0 if all ports are properly configured, otherwise the port number
